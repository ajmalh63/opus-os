...
| **P1-3 (substantial)** | Realtime publishes added at the client-facing mutation surface: manpower deployment created/updated (`client:{id}:applications` + `staff:global:manpower`), job posted (`public:manpower` + staff), payment verified (`client:{id}:payments`), visa submission (`staff:global:visa` + `client:{id}:visa`). **Bug found & fixed**: Hono `c.executionCtx` *getter throws* when no ExecutionContext (tests/non-Workers) → "This context has no ExecutionContext" 500s; all 5 publish sites (incl. the earlier membership-grant) now use the codebase's own `safeExecutionCtx(c)`. Subscribed-channel audit: UI currently subscribes helpdesk `client:{id}:tickets` + staff:global subset — client-side subscription rollout for the new channels is the identified tail (P2-1) | **122/122 · 757/757** ✓ |
...
**Still open (owner-dependent or scheduled):** ~~P0-2~~ (owner deprioritized) · ~~P1-1~~ ✅ · ~~P1-3~~ backend surface + safeExecutionCtx fix ✅ · ~~P2-4~~ ✅ · ~~P2-1~~ ✅ portal split (visa extracted + lazy; remaining tabs already use section components) · P2-6 apiClient (core built; adopt in remaining components) — plus client-side subscription rollout for new realtime channels (identified P2-1 tail) and Week 3–4 features. **Fixed: P1-2, P1-5, P2-3, P2-6 core, P2-7, P1-1, P2-2 (code-side), P1-3, P2-1.**
