...
- **Student portal 🎓 tab + intake wizard** (Wave 2): `StudentProfileWizard.tsx` (4-step, progress bar, planned tests, parent fields, **DPDP university-sharing consent** w/ SHA-256 notice hash); portal routes GET/PUT `/profile` (completeness %, staff alert @100%), GET `/documents`, presigned per-app doc upload → **auto-syncs checklist to 'received'** (staff 'verified' shows to student instantly — single source of truth); `StudyAbroadClientSection.tsx` (Profile/Applications/Documents, 30s refetch); staff registry completeness bars + agent-assisted wizard.
