...
- [x] **H6. Rate Limiting & Verification on Visa Inquiry** — ✅ SHIPPED: Added rate limiting (15/hr), client token/id resolution, and tamper-evident audit logging (`VISA_INQUIRY_SUBMITTED`) to `POST /api/public/portal/visa/inquiry` in `apps/api/src/routes/portal.ts`
...
- [x] **H10. Session Absolute Timeout Middleware** — ✅ SHIPPED: Implemented NIST SP 800-63B / OWASP ASVS absolute session timeout in `apps/api/src/lib/auth/session.ts` (30-day absolute ceiling for clients, 7-day absolute ceiling for staff) with automatic session purge on expiry
