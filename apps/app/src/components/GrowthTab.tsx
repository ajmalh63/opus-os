...
export default function GrowthTab() {
  ...
  const bandColor = (b: string) => b === 'hot' ? 'text-brand-error' : b === 'warm' ? 'text-brand-warning' : 'text-slate-400';
  const bandBg = (b: string) => b === 'hot' ? 'bg-brand-error/15' : b === 'warm' ? 'bg-brand-warning/15' : 'bg-slate-800';
      ...
      <div className="bg-[#1C2541]/40 border border-slate-800 rounded-xl overflow-hidden">
        <div className="px-6 py-4 border-b border-slate-800 flex flex-wrap items-center justify-between gap-2">
          <div>
            ...
            <p className="text-xs text-slate-400 mt-0.5">Each interaction earns points → auto band. Gold standard: cold &lt;50 · warm 50–75 · hot &gt;75.</p>
          </div>
          <div className="flex gap-3 text-[10px]">
            {segData?.segments?.map(s => (
              <span key={s.name} className="px-2 py-1 rounded border border-slate-800 bg-slate-950 text-slate-300">
                {s.name}: <b className="text-white">{s.count ?? s.rules_json?.band}</b>
              </span>
            ))}
          </div>
        </div>
        <table className="w-full text-left border-collapse text-xs">
          ...
          <tbody>
            {leadsData?.leads.map(l => (
              <tr key={l.clientId} className="border-b border-slate-800/60 hover:bg-slate-900/40">
                <td className="p-4">
                  <p className="font-semibold text-white">{l.name}</p>
                  <p className="text-[10px] text-slate-500">{l.clientId}</p>
                </td>
                <td className="p-4 font-mono font-bold text-white">{l.score}</td>
                <td className="p-4">
                  <span className={`px-2 py-0.5 rounded text-[9px] font-bold uppercase ${bandBg(l.band)} ${bandColor(l.band)}`}>{l.band}</span>
                </td>
                <td className="p-4 text-slate-400">{l.interactions}</td>
              </tr>
            ))}
            {!leadsLoading && (!leadsData || leadsData.leads.length === 0) && (
              <tr><td colSpan={4} className="p-8 text-center text-slate-500">No scored leads. Public interactions (lead form, consultation, WhatsApp reply) auto-score via /api/marketing/interactions.</td></tr>
            )}
          </tbody>
        </table>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-[#1C2541]/40 border border-slate-800 rounded-xl p-6 space-y-4">
          <div>
            ...
            <p className="text-xs text-slate-400 mt-0.5">Fixed amount (paise) per trigger per division — earnings auto-computed from real transactions.</p>
          </div>
          <form onSubmit={(e) => { e.preventDefault(); createRule.mutate(); }} className="flex flex-wrap gap-2">
            <select value={division} onChange={e => setDivision(e.target.value)} className="bg-slate-950 border border-slate-800 rounded px-2 py-2 text-xs text-white">
              {['study-abroad', 'visa', 'umrah', 'attestation', 'manpower'].map(d => <option key={d} value={d}>{d}</option>)}
            </select>
            <select value={trigger} onChange={e => setTrigger(e.target.value)} className="bg-slate-950 border border-slate-800 rounded px-2 py-2 text-xs text-white">
            ...
            <input type="number" min="1" step="0.01" value={amountRs} onChange={e => setAmountRs(e.target.value)} placeholder="₹ per event" className="w-28 bg-slate-950 border border-slate-800 rounded px-2 py-2 text-xs text-white placeholder-slate-600" />
          ...
          <div className="space-y-1.5">
            {rulesData?.rules.map(r => (
              <div key={r.id} className="flex justify-between items-center bg-slate-950 border border-slate-800 rounded px-3 py-2 text-xs">
                <span className="text-white font-semibold">{r.division} · {r.trigger}</span>
        ...
        <div className="bg-[#1C2541]/40 border border-slate-800 rounded-xl p-6 space-y-4">
          <div>
            <h3 className="font-display font-bold text-sm text-white">Payout Statements</h3>
            <p className="text-xs text-slate-400 mt-0.5">Close a period → accruals are frozen (no double-count), statements drafted with TDS (<code className="text-brand-gold">10%</code>).</p>
          </div>
          <form onSubmit={(e) => { e.preventDefault(); closePeriod.mutate(); }} className="flex items-end gap-2">
            <div className="flex-1">
              <label className="text-[10px] text-slate-400 font-bold uppercase block mb-1">Period (YYYY-MM)</label>
              <input type="month" value={period} onChange={e => setPeriod(e.target.value)} className="w-full bg-slate-950 border border-slate-800 rounded px-3 py-2 text-xs text-white" />
            </div>
          ...
          <div className="space-y-1.5">
            {stmtData?.statements.map(s => (
              <div key={s.id} className="flex justify-between items-center bg-slate-950 border border-slate-800 rounded px-3 py-2 text-xs">
                <span className="text-white font-semibold">{s.employeeId} · {s.period}</span>
                <span className="flex gap-3">
                  <span className="text-slate-400">Gross <b className="text-white">₹{(s.gross / 100).toFixed(2)}</b></span>
                  <span className="text-slate-400">Net <b className="text-brand-success">₹{(s.net / 100).toFixed(2)}</b></span>
                  <span className={`px-2 py-0.5 rounded text-[9px] uppercase font-bold ${s.status === 'approved' ? 'bg-brand-success/15 text-brand-success' : 'bg-slate-700 text-slate-200'}`}>{s.status}</span>
                </span>
              </div>
            ))}
            {(!stmtData || stmtData.statements.length === 0) && <p className="text-[10px] text-slate-500 text-center py-3">No payout statements yet. Close a period after accruals exist.</p>}
          </div>
        </div>
      </div>
    </div>
