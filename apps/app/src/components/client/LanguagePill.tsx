...
export default function LanguagePill({
  ...
  if (displayLangs.length === 1) {
    ...
    return (
      <div className="inline-flex items-center gap-1.5 text-[11px] font-bold border border-slate-200 bg-white rounded-full px-2.5 py-1 shadow-xs" aria-label={`Language: ${labelFor(active)}`}>
        <span className="w-3 h-3 rounded-full bg-slate-100 grid place-items-center text-[8px]" aria-hidden>🌐</span>
        {labelFor(active)}
      </div>
    );
  }
  ...
  return (
    <div className="relative">
      <button
        onClick={() => setOpen((v) => !v)}
        className="inline-flex items-center gap-1.5 text-[11px] font-bold border border-slate-200 bg-white rounded-full px-2.5 py-1 shadow-xs hover:border-slate-300 focus:outline-none focus-visible:ring-2 focus-visible:ring-brand-navy/20 min-h-[28px] min-w-[44px] justify-center cursor-pointer"
        aria-haspopup="listbox"
        aria-expanded={open}
        aria-label={`Language selector, current: ${labelFor(active)}`}
      >
        <span className="w-3 h-3 rounded-full bg-slate-100 grid place-items-center text-[8px]" aria-hidden>🌐</span>
        {labelFor(active)} <span className="text-slate-400 text-[10px]" aria-hidden>▾</span>
      </button>
      {open && (
        <div
          role="listbox"
