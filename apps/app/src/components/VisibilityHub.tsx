...
function SeoTab() {
  ...
  const { data, isLoading } = useQuery<any>({ queryKey: ['seoPages'], queryFn: async () => (await fetch('/api/visibility/seo/pages', { headers: AUTH })).json() });
  const { data: audit } = useQuery<any>({ queryKey: ['seoAudit'], queryFn: async () => (await fetch('/api/visibility/seo/audit', { headers: AUTH })).json() });
  const { data: kw } = useQuery<any>({ queryKey: ['seoKeywords'], queryFn: async () => (await fetch('/api/visibility/seo/keywords', { headers: AUTH })).json() });
  ...
  const savePage = useMutation({
    mutationFn: async () => {
      const r = await fetch('/api/visibility/seo/pages', { method: 'POST', headers: { 'Content-Type': 'application/json', ...AUTH }, body: JSON.stringify({ route: editing.route, ...form }) });
  ...
  const addKeyword = useMutation({
    mutationFn: async () => {
      const r = await fetch('/api/visibility/seo/keywords', { method: 'POST', headers: { 'Content-Type': 'application/json', ...AUTH }, body: JSON.stringify({ ...kwForm, volume: kwForm.volume ? Number(kwForm.volume) : undefined, position: kwForm.position ? Number(kwForm.position) : undefined }) });
...
function AeoTab() {
  const qc = useQueryClient();
  const { data: checks } = useQuery<any>({ queryKey: ['aeoChecks'], queryFn: async () => (await fetch('/api/visibility/aeo/checks', { headers: AUTH })).json() });
  const { data: passages } = useQuery<any>({ queryKey: ['aeoPassages'], queryFn: async () => (await fetch('/api/visibility/aeo/passages', { headers: AUTH })).json() });
  ...
  const runCheck = async () => {
    ...
    setRunning(true);
    try {
      const r = await fetch('/api/visibility/aeo/check', { method: 'POST', headers: { 'Content-Type': 'application/json', ...AUTH }, body: JSON.stringify({ query, engine }) });
  ...
  const addPassage = useMutation({
    mutationFn: async () => {
      const r = await fetch('/api/visibility/aeo/passages', { method: 'POST', headers: { 'Content-Type': 'application/json', ...AUTH }, body: JSON.stringify(pForm) });
...
function GbpTab() {
  const qc = useQueryClient();
  const { data: prof } = useQuery<any>({ queryKey: ['gbpProfile'], queryFn: async () => (await fetch('/api/visibility/gbp/profile', { headers: AUTH })).json() });
  const { data: posts } = useQuery<any>({ queryKey: ['gbpPosts'], queryFn: async () => (await fetch('/api/visibility/gbp/posts', { headers: AUTH })).json() });
  const [form, setForm] = useState<any>({});
  const [postForm, setPostForm] = useState({ title: '', body: '', status: 'draft' });

  const save = useMutation({
    mutationFn: async () => {
      const r = await fetch('/api/visibility/gbp/profile', { method: 'POST', headers: { 'Content-Type': 'application/json', ...AUTH }, body: JSON.stringify(form) });
  ...
  const addPost = useMutation({
    mutationFn: async () => {
      const r = await fetch('/api/visibility/gbp/posts', { method: 'POST', headers: { 'Content-Type': 'application/json', ...AUTH }, body: JSON.stringify(postForm) });
...
function GscTab() {
  const qc = useQueryClient();
  const { data } = useQuery<any>({ queryKey: ['gscQueries'], queryFn: async () => (await fetch('/api/visibility/search-console/queries', { headers: AUTH })).json() });
  const [form, setForm] = useState({ keyword: '', impressions: '', clicks: '', position: '' });
  const add = useMutation({
    mutationFn: async () => {
      const r = await fetch('/api/visibility/seo/keywords', { method: 'POST', headers: { 'Content-Type': 'application/json', ...AUTH }, body: JSON.stringify({ keyword: form.keyword, impressions: Number(form.impressions) || 0, clicks: Number(form.clicks) || 0, position: form.position ? Number(form.position) : undefined }) });
...
function ReviewsTab() {
  const qc = useQueryClient();
  const { data } = useQuery<any>({ queryKey: ['gbpReviews'], queryFn: async () => (await fetch('/api/visibility/gbp/reviews', { headers: AUTH })).json() });
  const [form, setForm] = useState({ source: 'google', rating: 5, author: '', text: '' });
  const [drafts, setDrafts] = useState<Record<string, string>>({});

  const add = useMutation({
    mutationFn: async () => {
      const r = await fetch('/api/visibility/gbp/reviews', { method: 'POST', headers: { 'Content-Type': 'application/json', ...AUTH }, body: JSON.stringify(form) });
  ...
  const respond = useMutation({
    mutationFn: async (id: string) => {
      const r = await fetch(`/api/visibility/gbp/reviews/${id}/respond`, { method: 'POST', headers: { 'Content-Type': 'application/json', ...AUTH }, body: JSON.stringify({ responseDraft: drafts[id] || '' }) });
...
function AttributionTab() {
  const { data } = useQuery<any>({ queryKey: ['attribution'], queryFn: async () => (await fetch('/api/visibility/attribution', { headers: AUTH })).json() });
...
function ReportsTab() {
  const qc = useQueryClient();
  const { data } = useQuery<any>({ queryKey: ['reportSchedules'], queryFn: async () => (await fetch('/api/visibility/reports/schedules', { headers: AUTH })).json() });
  const [form, setForm] = useState({ name: '', reportType: 'revenue', period: 'monthly', recipients: '' });
  const add = useMutation({
    mutationFn: async () => {
      const r = await fetch('/api/visibility/reports/schedules', { method: 'POST', headers: { 'Content-Type': 'application/json', ...AUTH }, body: JSON.stringify(form) });
  ...
  const runNow = useMutation({
    mutationFn: async (id: string) => {
      const r = await fetch(`/api/visibility/reports/schedules/${id}/run`, { method: 'POST', headers: AUTH });
