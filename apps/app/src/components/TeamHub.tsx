...
export default function TeamHub() {
  ...
  return (
    <div className="grid min-h-full grid-cols-1 gap-5 lg:grid-cols-3">
      <div className="lg:col-span-2 rounded-2xl border border-brand-navy/10 bg-white shadow-[0_20px_40px_-20px_rgba(10,45,80,0.12)]">
        <div className="flex flex-wrap items-center gap-2 border-b border-brand-navy/10 px-5 py-3">
          ...
          {ROOMS.map((r) => (
            <button key={r.id} onClick={() => { setRoomId(r.id); setAfter(0); }}
              className={`inline-flex cursor-pointer items-center gap-1.5 rounded-full px-3 py-1.5 text-[11px] font-bold transition-all ${roomId === r.id ? 'bg-brand-navy text-white' : 'border border-brand-navy/15 text-slate-600 hover:border-brand-gold hover:text-brand-gold'}`}>
        ...
        <div ref={boxRef} className="h-[46vh] space-y-3 overflow-y-auto px-5 py-4">
          {all.length === 0 && <p className="pt-10 text-center text-xs text-slate-400">No messages yet — start the conversation.</p>}
          {all.map((m) => (
            <div key={m.id} className={`flex ${m.senderName === meName ? 'justify-end' : 'justify-start'}`}>
              <div className={`max-w-[75%] rounded-2xl px-3.5 py-2 text-xs ${m.senderName === meName ? 'bg-brand-navy text-white' : 'border border-brand-navy/10 bg-slate-50 text-brand-navy'}`}>
                <div className="mb-0.5 flex items-baseline gap-2">
                  <span className={`text-[10px] font-bold uppercase tracking-wider ${m.senderName === meName ? 'text-brand-gold' : 'text-slate-500'}`}>{m.senderName}</span>
                  <span className="text-[9px] opacity-60">{new Date(m.ts * 1000).toLocaleTimeString()}</span>
                </div>
        ...
        <div className="flex gap-2 border-t border-brand-navy/10 px-5 py-3">
          <input value={draft} onChange={(e) => setDraft(e.target.value)}
            ...
            className="flex-1 rounded-xl border border-brand-navy/15 bg-slate-50 px-3.5 py-2.5 text-xs text-brand-navy placeholder:text-slate-400 focus:border-brand-gold focus:outline-none" />
          <button onClick={() => send.mutate()} disabled={send.isPending || !draft.trim()}
            className="cursor-pointer rounded-xl bg-brand-gold px-5 py-2.5 text-xs font-bold uppercase tracking-wider text-brand-navy transition-all hover:bg-brand-1 active:scale-[0.97] disabled:opacity-40">
            Send
          </button>
        </div>
      </div>

      <div className="rounded-2xl border border-brand-navy/10 bg-white shadow-[0_20px_40px_-20px_rgba(10,45,80,0.12)]">
        <div className="border-b border-brand-navy/10 px-5 py-3">
          ...
          <p className="text-[10px] text-slate-400">Shared files in R2 (team/)</p>
        </div>
        <div className="max-h-[46vh] space-y-1.5 overflow-y-auto px-3 py-3">
          {(files?.files || []).length === 0 && <p className="px-2 pt-8 text-center text-xs text-slate-400">No shared files yet.</p>}
          {(files?.files || []).map((f) => (
            <div key={f.key} className="flex items-center gap-2 rounded-lg border border-brand-navy/5 bg-slate-50 px-3 py-2 text-[11px]">
              ...
              <span className="truncate text-slate-700">{f.key.replace('team/', '')}</span>
              <span className="ml-auto font-mono text-[9px] text-slate-400">{(f.size / 1024).toFixed(0)}KB</span>
            </div>
          ))}
        </div>
      </div>
