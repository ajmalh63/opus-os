  ...
  return (
    <div className="rounded-2xl border border-brand-navy/10 bg-white p-4">
      <div className="flex flex-wrap items-center gap-2">
        <label className="cursor-pointer rounded-lg bg-brand-navy px-3 py-2 text-[11px] font-bold text-white hover:bg-brand-navy/90">
          📎 Choose scan…
      ...
      {result && (
        <div className="mt-3">
          <div className="flex items-center justify-between">
            <p className="text-[10px] font-bold uppercase tracking-wider text-brand-gold">Extracted text</p>
            <button
              onClick={() => navigator.clipboard?.writeText(result)}
              className="text-[10px] font-bold text-brand-navy/50 hover:text-brand-gold"
            >
              Copy
            </button>
          </div>
          <pre className="mt-1 max-h-48 overflow-y-auto whitespace-pre-wrap rounded-lg bg-slate-50 p-3 text-[11px] leading-relaxed text-brand-navy">
            {result}
          </pre>
        </div>
      )}
