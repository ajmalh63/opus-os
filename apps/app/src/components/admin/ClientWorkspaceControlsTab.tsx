...
function AttestationSuperadminForm({ selectedClientId, onDone }: { selectedClientId: string; onDone: () => void }) {
  ...
  return (
    <div className="rounded-xl border border-brand-navy/10 bg-brand-navy/[0.02] p-4 space-y-3">
      ...
      <div className="text-[10px] text-brand-navy/40">All 42 destinations available — same dropdown client sees. Creates `quote_requested` stage; staff desk moves `quote_confirmed → docs_awaiting → in_process → completed → dispatched → delivered` with chain steps; client tracker shows it live.</div>
    </div>
  );
}
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-gradient-to-r from-brand-navy via-[#0f2a4d] to-[#06182c] rounded-2xl p-6 border border-white/10 text-white flex flex-wrap items-center justify-between gap-4">
        <div>
          <div className="flex items-center gap-2"><span className="h-2 w-2 rounded-full bg-brand-gold animate-pulse" /><span className="text-[10px] font-bold uppercase tracking-widest text-brand-gold">Superadmin · Client Workspace Mirror</span></div>
        ...
        <div className="bg-white/10 border border-white/15 rounded-xl px-4 py-3 text-xs">
          <div className="text-white/50 text-[10px] uppercase tracking-wider font-bold">Selected Client</div>
          <div className="text-brand-gold font-bold">{selectedClient?.name || '— none —'}</div>
          <div className="text-white/50 font-mono text-[10px]">{selectedClientId || 'pick from directory'}</div>
        </div>
      </div>

      <div className="bg-white border border-brand-navy/10 rounded-2xl p-4 flex flex-wrap gap-3 items-end">
        <div className="flex-1 min-w-[240px]">
          <label className="text-[10px] font-bold uppercase tracking-wider text-brand-navy/40">Client directory (every portal token)</label>
        ...
        <div className="flex gap-1.5 bg-brand-navy/[0.05] p-1 rounded-xl">
          {([
            ...
            { k: 'visa', l: '✈️ Visa', d: 'Checklists + Slots' },
            ...
            <button key={t.k} onClick={() => setDivision(t.k as Division)} className={`px-3 py-2 rounded-lg text-xs font-bold text-left leading-tight ${division === t.k ? 'bg-brand-gold text-brand-navy shadow' : 'text-brand-navy/60 hover:text-brand-navy hover:bg-brand-navy/5'}`}>
              <div>{t.l}</div><div className="text-[9px] font-normal opacity-70">{t.d}</div>
            </button>
          ))}
        </div>
        ...
        <>
            ...
            <div className="space-y-6">
              <div className="rounded-2xl border border-brand-navy/10 bg-white p-6 space-y-4">
                <div className="flex items-center justify-between">
                  ...
                  <span className="text-[10px] px-2 py-1 rounded-full bg-emerald-50 border border-emerald-200 text-emerald-700 font-bold">Client Token: {selectedClientId}</span>
                </div>
                <p className="text-[11px] text-brand-navy/50">All 28 fields the client fills — you can edit, overwrite, or fill on behalf. Same 6-step wizard, same validation, same 60% gate that unlocks real Match%.</p>
                ...
                <div className="rounded-xl border border-dashed border-brand-navy/15 p-4 bg-brand-navy/[0.02] space-y-2">
                  ...
                  <div className="flex flex-wrap gap-2 text-[11px] text-brand-navy/60">
                  ...
                  <div className="text-[10px] text-brand-navy/40">Staff desk `Client360 → Documents` and `ManpowerPortal → Deployment` both read the same `documents` + `intakeContext.manpowerProfile.resumeKey` — delete here removes from portal instantly.</div>
                </div>
              </div>
              ...
              <div className="rounded-2xl border border-brand-navy/10 bg-white p-6 space-y-4">
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-3 text-xs">
                  <div><label className="text-[10px] font-bold uppercase tracking-wider text-brand-navy/40">Title *</label><input value={jobDraft.title} onChange={e => setJobDraft({ ...jobDraft, title: e.target.value })} placeholder="Structural Welder" className="w-full mt-1 rounded-lg border border-brand-navy/10 px-3 py-2" /></div>
                  <div><label className="text-[10px] font-bold uppercase tracking-wider text-brand-navy/40">Country *</label><input list="admin-countries" value={jobDraft.country} onChange={e => setJobDraft({ ...jobDraft, country: e.target.value })} className="w-full mt-1 rounded-lg border border-brand-navy/10 px-3 py-2" /><datalist id="admin-countries">{filterOptions.countries.split(',').map(s => s.trim()).map(c => <option key={c} value={c} />)}</datalist></div>
                  <div><label className="text-[10px] font-bold uppercase tracking-wider text-brand-navy/40">Sector *</label><input list="admin-sectors" value={jobDraft.sector} onChange={e => setJobDraft({ ...jobDraft, sector: e.target.value })} className="w-full mt-1 rounded-lg border border-brand-navy/10 px-3 py-2" /><datalist id="admin-sectors">{filterOptions.sectors.split(',').map(s => s.trim()).map(c => <option key={c} value={c} />)}</datalist></div>
                  <div><label className="text-[10px] font-bold uppercase tracking-wider text-brand-navy/40">Salary Text *</label><input value={jobDraft.salaryText} onChange={e => setJobDraft({ ...jobDraft, salaryText: e.target.value })} placeholder="QR 2,500" className="w-full mt-1 rounded-lg border border-brand-navy/10 px-3 py-2" /></div>
                  <div><label className="text-[10px] font-bold uppercase tracking-wider text-brand-navy/40">Collar</label><select value={jobDraft.collar} onChange={e => setJobDraft({ ...jobDraft, collar: e.target.value })} className="w-full mt-1 rounded-lg border border-brand-navy/10 px-3 py-2"><option value="blue_collar">Blue Collar</option><option value="white_collar">White Collar</option></select></div>
                  <div><label className="text-[10px] font-bold uppercase tracking-wider text-brand-navy/40">Tier</label><select value={jobDraft.tier} onChange={e => setJobDraft({ ...jobDraft, tier: e.target.value })} className="w-full mt-1 rounded-lg border border-brand-navy/10 px-3 py-2"><option value="public">Public</option><option value="secret">Secret (exclusive)</option></select></div>
                  <div><label className="text-[10px] font-bold uppercase tracking-wider text-brand-navy/40">Experience min (yrs)</label><input type="number" value={jobDraft.experienceYearsMin} onChange={e => setJobDraft({ ...jobDraft, experienceYearsMin: Number(e.target.value) })} className="w-full mt-1 rounded-lg border border-brand-navy/10 px-3 py-2" /></div>
                  <div><label className="text-[10px] font-bold uppercase tracking-wider text-brand-navy/40">Trade category</label><input value={jobDraft.tradeCategory} onChange={e => setJobDraft({ ...jobDraft, tradeCategory: e.target.value })} placeholder="Welding, Nursing…" className="w-full mt-1 rounded-lg border border-brand-navy/10 px-3 py-2" /></div>
                  <div className="col-span-2"><label className="text-[10px] font-bold uppercase tracking-wider text-brand-navy/40">Employer</label><input value={jobDraft.employer} onChange={e => setJobDraft({ ...jobDraft, employer: e.target.value })} className="w-full mt-1 rounded-lg border border-brand-navy/10 px-3 py-2" /></div>
                  <div className="col-span-2"><label className="text-[10px] font-bold uppercase tracking-wider text-brand-navy/40">Requirements (comma)</label><input value={jobDraft.requirements} onChange={e => setJobDraft({ ...jobDraft, requirements: e.target.value })} placeholder="MIG, TIG, Blueprint" className="w-full mt-1 rounded-lg border border-brand-navy/10 px-3 py-2" /></div>
                  <div className="col-span-2"><label className="text-[10px] font-bold uppercase tracking-wider text-brand-navy/40">Benefits (comma)</label><input value={jobDraft.benefits} onChange={e => setJobDraft({ ...jobDraft, benefits: e.target.value })} placeholder="Accommodation, Food, Visa" className="w-full mt-1 rounded-lg border border-brand-navy/10 px-3 py-2" /></div>
                  <div className="col-span-2"><label className="text-[10px] font-bold uppercase tracking-wider text-brand-navy/40">Description</label><textarea value={jobDraft.description} onChange={e => setJobDraft({ ...jobDraft, description: e.target.value })} rows={2} className="w-full mt-1 rounded-lg border border-brand-navy/10 px-3 py-2" /></div>
                </div>
                ...
                <div className="border-t border-brand-navy/10 pt-4">
                  <div className="text-[11px] font-bold text-brand-navy mb-2">Live postings (edit any field inline via Manpower Portal, or delete here): {jobsData?.jobs?.length || 0}</div>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-3 max-h-[320px] overflow-auto">
                    {(jobsData?.jobs || []).slice(0, 8).map((j: any) => (
                      <div key={j.id} className="rounded-xl border border-brand-navy/10 bg-brand-navy/[0.03] p-3 flex items-center justify-between">
                        <div><div className="font-bold text-xs text-brand-navy">{j.title}</div><div className="text-[10px] text-brand-navy/50">{j.country} · {j.sector} · {j.collar} · {j.tier} · {j.salaryText}</div></div>
                        <button onClick={async () => { if (confirm('Archive job?')) { await fetch(`/api/manpower/jobs/${j.id}`, { method: 'DELETE' }); qc.invalidateQueries({ queryKey: ['adminManpowerJobs'] }); showToast('Archived'); }}} className="text-[10px] font-bold text-rose-600 border border-rose-200 bg-rose-50 px-2 py-1 rounded">Archive</button>
                      </div>
                    ))}
                  </div>
                </div>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
                <div className="rounded-2xl border border-brand-navy/10 bg-white p-5 space-y-3">
                  ...
                  <div><label className="text-[10px] font-bold uppercase tracking-wider text-brand-navy/40">Countries (comma — powers client filter + job form datalist)</label><textarea value={filterOptions.countries} onChange={e => setFilterOptions({ ...filterOptions, countries: e.target.value })} rows={2} className="w-full mt-1 rounded-lg border border-brand-navy/10 px-3 py-2 text-xs" /></div>
                  <div><label className="text-[10px] font-bold uppercase tracking-wider text-brand-navy/40">Sectors</label><textarea value={filterOptions.sectors} onChange={e => setFilterOptions({ ...filterOptions, sectors: e.target.value })} rows={2} className="w-full mt-1 rounded-lg border border-brand-navy/10 px-3 py-2 text-xs" /></div>
                  <div className="text-[10px] text-brand-navy/40">Persist via <code>app_settings</code> `manpower_filters` — hot-reloads marketplace without deploy.</div>
                </div>
                <div className="rounded-2xl border border-brand-navy/10 bg-white p-5 space-y-3">
                ...
                <div className="rounded-2xl border border-brand-navy/10 bg-white p-5 space-y-3">
                  ...
                  <div className="text-xs space-y-1 text-brand-navy/70">
                    ...
                    <div>Tiers: <span className="px-1.5 py-0.5 rounded bg-emerald-50 border border-emerald-200 text-emerald-700 text-[10px] font-bold">top_match ≥75</span> <span className="px-1.5 py-0.5 rounded bg-amber-50 border border-amber-200 text-amber-700 text-[10px] font-bold">standard ≥50</span> <span className="px-1.5 py-0.5 rounded bg-slate-100 border text-[10px] font-bold">cold_pool &lt;50</span></div>
                    <div className="text-[10px] text-brand-navy/50">Engine file: <code>apps/api/src/lib/manpowerMatch.ts → computeManpowerMatch()</code>. Portal uses <code>apps/app/src/lib/manpowerMatch.ts</code> mirror — edit both to retune. 60% completeness gates real score (else 35% demo).</div>
          ...
          {division === 'studyAbroad' && (
            <div className="rounded-2xl border border-brand-navy/10 bg-white p-6 space-y-4">
              ...
              <div className="text-[11px] text-brand-navy/40">Also mirrored: <a href="/study-abroad" className="underline text-brand-gold">StudyAbroadPortal</a> — shortlist, snapshot, offer pipeline.</div>
            </div>
          )}
          ...
          {division === 'attestation' && (
            <div className="rounded-2xl border border-brand-navy/10 bg-white p-6 space-y-4">
              ...
              <div className="rounded-xl border border-brand-navy/10 bg-brand-navy/[0.02] p-3 text-[11px] text-brand-navy/60 space-y-1">
          ...
          {(division === 'visa' || division === 'umrah') && (
            <div className="rounded-2xl border border-dashed border-brand-navy/15 bg-white/60 p-8 text-center space-y-3">
              <h3 className="font-display font-black text-brand-navy">{division === 'visa' ? '✈️ Visa' : '🕋 Umrah'} — Client Controls</h3>
