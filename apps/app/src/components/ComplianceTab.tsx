  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <h2 className="font-display font-bold text-base text-white">GST & Statutory Compliance Workbench (Section 14.5)</h2>
        <input type="month" value={period} onChange={e => setPeriod(e.target.value)} className="bg-slate-950 border border-slate-800 rounded px-3 py-2 text-xs text-white" />
      </div>

      {/* Profile */}
      <div className="bg-[#1C2541]/40 border border-slate-800 rounded-xl p-5 flex flex-wrap items-end gap-3 text-xs">
        <div>
          <label className="text-[10px] text-slate-400 font-bold uppercase block mb-1">Legal name</label>
          <div className="bg-slate-950 border border-slate-800 rounded px-3 py-2 text-white">{prof?.profile?.legalName || '—'}</div>
        </div>
        <div>
          <label className="text-[10px] text-slate-400 font-bold uppercase block mb-1">GSTIN</label>
          <input value={gstin || prof?.profile?.gstin || ''} onChange={e => setGstin(e.target.value)} placeholder="GSTIN" className="bg-slate-950 border border-slate-800 rounded px-3 py-2 text-white w-52" />
        </div>
        <div>
          <label className="text-[10px] text-slate-400 font-bold uppercase block mb-1">State</label>
          <div className="bg-slate-950 border border-slate-800 rounded px-3 py-2 text-white">{prof?.profile?.stateName || '—'}</div>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
        <div className="bg-[#1E1F41]/40 border border-slate-800 rounded-xl p-5 space-y-3">
          <h3 className="font-display font-bold text-sm text-white">GSTR-1 Export</h3>
          <p className="text-[10px] text-slate-400">GSTN offline-tool JSON v1.7 — B2B, B2C, HSN, credit notes, doc summary.</p>
          ...
          {g1?.stats && <div className="text-[10px] text-emerald-400">{g1.stats.b2bInvoices} B2B · {g1.stats.b2cLines} B2C · {g1.stats.hsnLines} HSN</div>}
        </div>
        <div className="bg-[#1E1F41]/40 border border-slate-800 rounded-xl p-5 space-y-3">
          <h3 className="font-display font-bold text-sm text-white">GSTR-3B Computation</h3>
          <p className="text-[10px] text-slate-400">Output tax, ITC from purchases, net payable — from D1.</p>
          ...
          {g3?.computed && (
            <div className="grid grid-cols-3 gap-2 text-[10px]">
              <div className="bg-slate-950 rounded p-2"><span className="text-slate-400 block">Output</span><b className="text-white">{rs(g3.computed.outputTax)}</b></div>
              <div className="bg-slate-950 rounded p-2"><span className="text-slate-400 block">ITC</span><b className="text-emerald-300">{rs(g3.computed.inputItc)}</b></div>
              <div className="bg-slate-950 rounded p-2"><span className="text-slate-400 block">Net</span><b className="text-brand-warning">{rs(g3.computed.netPayable)}</b></div>
            </div>
          )}
        </div>
      </div>
      ...
      {/* 2B */}
      <div className="bg-[#1E1F41]/40 border border-slate-800 rounded-xl p-5 space-y-3">
        <h3 className="font-display font-bold text-sm text-white">GSTR-2B Reconciliation</h3>
        <p className="text-[10px] text-slate-400">Import gst.gov.in 2B JSON → auto-match vs D1 purchase invoices.</p>
        ...
        {rec && (
          <div className="grid grid-cols-4 gap-2 text-[10px]">
            <div className="bg-emerald-950/40 border border-emerald-800 rounded-lg p-2 text-center"><b className="text-emerald-300">{rec.summary.matched}</b><span className="text-slate-400 block">Matched</span></div>
            <div className="bg-amber-950/40 border border-amber-800 rounded-lg p-2 text-center"><b className="text-amber-300">{rec.summary.mismatched}</b><span className="text-slate-400 block">Mismatch</span></div>
            <div className="bg-rose-950/40 border border-rose-800 rounded-lg p-2 text-center"><b className="text-rose-300">{rec.summary.twoBOnly}</b><span className="text-slate-400 block">2B-only</span></div>
            <div className="bg-slate-950 border border-slate-800 rounded-lg p-2 text-center"><b className="text-white">{rec.summary.booksOnly}</b><span className="text-slate-400 block">Books-only</span></div>
          </div>
        )}
      </div>

      {/* TDS */}
      <div className="bg-[#1E1F41]/40 border border-slate-800 rounded-xl p-5 space-y-4">
        <h3 className="font-display font-bold text-sm text-brand-gold">TDS Register (new codes 1026–1028)</h3>
        <form onSubmit={(e) => { e.preventDefault(); if (v.trim() && g) addTds.mutate(); }} className="flex flex-wrap gap-2 text-xs">
          <input value={v} onChange={e => setV(e.target.value)} placeholder="Vendor / payee" className="w-44 bg-slate-950 border border-slate-800 rounded px-2 py-2 text-white placeholder-slate-600" />
          <select value={sec} onChange={e => setSec(e.target.value)} className="bg-slate-950 border border-slate-800 rounded px-2 py-2 text-white"><option value="194J">194J</option><option value="194C">194C</option><option value="194H">194H</option></select>
          <input type="number" value={g} onChange={e => setG(e.target.value)} placeholder="Gross ₹" className="w-24 bg-slate-950 border border-slate-800 rounded px-2 py-2 text-white placeholder-slate-600" />
          <input type="number" value={t} onChange={e => setT(e.target.value)} placeholder="TDS ₹" className="w-24 bg-slate-950 border border-slate-800 rounded px-2 py-2 text-white placeholder-slate-600" />
        ...
        <div className="space-y-1.5">
          {reg?.tds?.map((r: any) => (
            <div key={r.id} className="flex justify-between items-center bg-slate-950 border border-slate-800 rounded px-3 py-2 text-xs">
              <span className="text-white font-semibold">{r.vendor_name} <span className="text-slate-400">· {r.section} · code {r.code}</span></span>
