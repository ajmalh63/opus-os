  ...
  return (
    ...
    >
      <div className="flex items-start justify-between gap-2 text-brand-navy">
        ...
        <span className={`shrink-0 rounded px-1 py-0.5 text-[9px] font-bold uppercase ${PRIORITY_STYLE[t.priority] || PRIORITY_STYLE.medium}`}>{t.priority}</span>
      </div>
      <div className="mt-1 flex flex-wrap items-center gap-1.5 text-[10px] text-brand-navy/50">
      ...
      {blocked && <div className="mt-1 truncate text-[10px] text-rose-700">blocked: {t.blockedReason}</div>}
      <div className="mt-1.5 flex flex-wrap gap-1.5">
        {(['block', 'expedite', 'clear'] as const).map((a) => {
          ...
          return (
            <button
              ...
              className="rounded-full border border-brand-navy/15 px-1.5 py-0.5 text-[9px] font-bold uppercase text-brand-navy/50 hover:border-rose-500 hover:text-rose-700"
            >
        ...
        {t.status === 'done' && (
          <button onClick={(e) => { e.stopPropagation(); if (confirm('Archive this card?')) move.mutate({ id: t.id, patch: { status: 'cancelled' } }); }} className="rounded-full border border-brand-navy/15 px-1.5 py-0.5 text-[9px] font-bold uppercase text-brand-navy/50 hover:bg-rose-600 hover:text-white">archive</button>
        )}
      </div>
    </div>
  );
    ...
    <div ref={rootRef} className="space-y-5 p-6 text-brand-navy">
      <div className="reveal flex flex-wrap items-center justify-between gap-3">
        <div>
          <div className="flex items-center gap-2.5">
            ...
            <p className="text-[10px] font-bold uppercase tracking-[0.18em] text-brand-gold">Boards</p>
          </div>
          <h2 className="mt-2 font-display text-sm font-bold text-brand-navy">Task Boards</h2>
          <p className="mt-1 text-[11px] text-brand-navy/40">Kanban system · {data.staffTotal} staff · finish-before-start enforced by WIP limits; expedite bypasses with limit {data.prefs.expediteLimit}.</p>
        </div>
        <div className="flex flex-wrap items-center gap-2.5">
          ...
          {isManager && (
            <button onClick={() => { setSettings({ columnLimitInProgress: data.prefs.columnLimitInProgress, personLimitInProgress: data.prefs.personLimitInProgress, expediteLimit: data.prefs.expediteLimit, policyText: data.prefs.policyText }); setShowSettings(true); }}
              className="rounded-full border border-brand-navy/15 bg-brand-navy/[0.04] px-3 py-2 text-[10px] font-bold uppercase text-brand-navy/70 hover:border-brand-gold/50 hover:text-brand-gold">⚙ Policies &amp; limits</button>
          )}
          <button onClick={() => setShowPolicies((v) => !v)} className="rounded-full border border-brand-navy/15 bg-brand-navy/[0.04] px-3 py-2 text-[10px] font-bold uppercase text-brand-navy/70 hover:border-brand-gold/50 hover:text-brand-gold">Policies</button>
          <button onClick={() => setMyOnly((v) => !v)} className={`rounded-full px-4 py-2 text-[11px] font-extrabold uppercase transition ${myOnly ? 'bg-brand-1 text-brand-navy' : 'border border-brand-navy/15 bg-brand-navy/[0.04] text-brand-navy/70'}`}>My tasks ({myCount})</button>
          <button onClick={() => setShowCreate((v) => !v)} className="rounded-full bg-brand-gold px-4 py-2 text-[11px] font-extrabold uppercase text-brand-navy hover:bg-brand-gold/90">{showCreate ? 'Close' : '+ New task'}</button>
        </div>
      </div>
      ...
      <div className="reveal grid grid-cols-2 gap-2 md:grid-cols-5">
        {[
          ...
          <div key={k} className={`rounded-xl border px-3 py-2 ${overCap && k === 'Active WIP' ? 'border-rose-500/40 bg-rose-500/10' : 'border-brand-navy/10 bg-white'}`}>
            <div className="text-[9px] font-bold uppercase tracking-widest text-brand-navy/40">{k}</div>
      ...
      {showPolicies && (
        <div className="rounded-2xl border border-brand-navy/10 bg-white p-4">
          <div className="text-[10px] font-bold uppercase tracking-widest text-brand-navy/40">Explicit policies (Definition of Done)</div>
          <p className="mt-1.5 text-[11px] leading-relaxed text-brand-navy/40">{data.prefs.policyText}</p>
        </div>
      )}

      {data.metrics.blocked.length > 0 && (
        <div className="rounded-2xl border border-rose-500/40 bg-rose-500/10 px-4 py-2.5 text-[11px] text-rose-700">
      ...
      <section className={`reveal rounded-2xl border-t-4 border-t-rose-400 border-b border-x border-brand-navy/10 bg-rose-500/10 p-3 ${expediteActive.length === 0 ? 'hidden' : ''}`}>
        <div className="flex items-center justify-between px-1 pb-2">
          <span className="text-[10px] font-bold uppercase tracking-widest text-rose-700">⚡ Expedite lane · {expediteActive.length}/{data.prefs.expediteLimit}</span>
          {inLaneCount('expedite') >= data.prefs.expediteLimit && <span className="text-[10px] font-bold text-rose-600">lane full — limit reached</span>}
        </div>
        ...
        <div className="mt-1.5 text-[10px] text-brand-navy/40">Expedite bypasses the column WIP cap (gold standard: max 1, all-hands on it).</div>
      </section>
      ...
      <div className="reveal grid grid-cols-1 gap-4 md:grid-cols-3">
        {COLUMNS.map((col) => {
          ...
          return (
            ...
            >
              <div className="flex items-center justify-between px-1 pb-2">
                <span className="text-[10px] font-bold uppercase tracking-widest text-brand-navy/40">{col.label}</span>
                <span className={`rounded-full px-2 py-0.5 text-[10px] font-bold ${full ? 'bg-rose-500/15 text-rose-700' : 'bg-brand-navy/[0.05] text-brand-navy/50'}`}>
                  {isCapped ? `${colCount}/${cap}` : colCount}
                </span>
              </div>
              {isCapped && full && (
                <div className="mb-2 rounded-lg border border-rose-500/40 bg-rose-500/10 px-2 py-1 text-[10px] font-bold text-rose-700">WIP cap reached — finish before starting (expedite excepted)</div>
              )}
              <div className="space-y-2">
                ...
                {colTasks.length === 0 && <div className="rounded-xl border border-dashed border-brand-navy/15 py-6 text-center text-[10px] text-brand-navy/50">Drop tasks here</div>}
              </div>
            </section>
          );
        })}
          ...
          <aside className="fixed top-0 right-0 h-full w-96 bg-white shadow-2xl z-50 p-8 flex flex-col justify-between border-l border-brand-navy/10 transition-transform duration-300 overflow-y-auto">
            <div className="flex flex-col gap-6 text-brand-navy">
              <div className="flex justify-between items-start border-b border-brand-navy/10 pb-4">
                <div>
                  <span className="text-[9px] font-bold text-brand-gold uppercase tracking-widest">Task Details</span>
                  ...
                  <p className="text-[9px] text-brand-navy/40 mt-0.5">Task ID: <span className="font-mono text-brand-navy font-semibold">{selectedTask.id.slice(0, 8)}</span></p>
                </div>
                <button 
                  onClick={() => setSelectedTaskId(null)}
              ...
              <div className="space-y-4">
                ...
                <div className="flex flex-col gap-1 py-1 border-b border-brand-navy/[0.08]">
                  <label className="text-[9px] uppercase text-brand-navy/40 font-bold">Task Title</label>
                  <input
                    type="text"
                    value={editTitle}
                ...
                <div className="flex flex-col gap-1 py-1 border-b border-brand-navy/[0.08]">
                  <label className="text-[9px] uppercase text-brand-navy/40 font-bold">Description</label>
                  <textarea
                    value={editDesc}
              ...
              <div className="space-y-2 border rounded-xl border-rose-200 bg-rose-50/20 p-4 mt-2">
                ...
                <p className="text-[9px] text-rose-700/80">Permanently delete this operation task from the database. This cannot be undone.</p>
                <button
                  type="button"
                  onClick={() => {
      ...
      {showCreate && (
        <div className="flex flex-wrap items-center gap-2 rounded-2xl border border-brand-navy/10 bg-white p-4">
          ...
          <button onClick={() => create.mutate()} disabled={create.isPending || form.title.trim().length < 2} className="rounded-full bg-brand-gold px-4 py-2 text-[11px] font-bold uppercase text-brand-navy hover:bg-brand-gold/90 disabled:opacity-40">
      ...
      {showSettings && isManager && (
        <div className="rounded-2xl border border-brand-navy/10 bg-white p-4 space-y-3">
          <div className="text-[10px] font-bold uppercase tracking-widest text-brand-navy/40">System settings (audited — experiment trail: change, observe 2 weeks, adjust on data)</div>
          <div className="grid grid-cols-3 gap-3">
            <label className="text-[11px] text-brand-navy/40">In-progress WIP cap
            ...
            <label className="text-[11px] text-brand-navy/40">Person WIP cap (soft)
            ...
            <label className="text-[11px] text-brand-navy/40">Expedite lane cap
          ...
          <button onClick={() => savePrefs.mutate()} disabled={savePrefs.isPending} className="rounded-full bg-brand-gold px-4 py-2 text-[11px] font-extrabold uppercase text-brand-navy hover:bg-brand-gold/90">
