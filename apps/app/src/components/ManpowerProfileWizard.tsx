  ...
  return (
    <div className="rounded-2xl border border-brand-navy/10 bg-white p-5 shadow-sm space-y-4">
      <div className="flex items-center justify-between">
        <h3 className="font-display font-bold text-brand-navy text-sm">{title || '🌍 Complete Your Career Profile'}</h3>
      ...
      <div>
        <div className="flex justify-between items-center mb-1.5">
          <div className="flex gap-1.5 flex-wrap">
            {STEPS.map((s, i) => (
              <button key={s} onClick={() => i < step && setStep(i)} className={`px-2.5 py-1 rounded-full text-[9px] font-bold cursor-pointer transition-all ${i === step ? 'bg-brand-gold text-brand-navy' : i < step ? 'bg-emerald-500/15 text-emerald-700' : 'bg-brand-navy/[0.06] text-brand-navy/40'}`}>
                {i < step ? '✓ ' : ''}{s}
              </button>
            ))}
          </div>
          <span className="text-[10px] font-bold text-brand-navy/50">{comp.pct}% complete</span>
        </div>
        <div className="h-2 rounded-full bg-brand-navy/[0.08] overflow-hidden"><div className="h-full bg-brand-gold transition-all duration-500" style={{ width: `${Math.max(20, comp.pct)}%` }} /></div>
      </div>

      <div className="min-h-[320px]">
        {step === 0 && (
          <div className="space-y-3">
            <p className="text-[10px] text-brand-navy/50">Who you are — all fields optional, fill what you have.</p>
        ...
        {step === 1 && (
          <div className="space-y-3">
            <p className="text-[10px] text-brand-navy/50">How we reach you + passport readiness for Gulf deployment.</p>
            <div className="grid grid-cols-2 gap-3">
              ...
              <div className="col-span-2 rounded-lg border border-brand-navy/10 p-3 space-y-3 bg-brand-navy/[0.02]">
                <label className="flex items-center gap-2 text-brand-navy/70 cursor-pointer"><input type="checkbox" checked={!!form.hasPassport} onChange={e => set('hasPassport', e.target.checked)} className="h-4 w-4 accent-brand-gold" /><span className="text-xs font-semibold">I have a valid passport</span></label>
                {form.hasPassport && (
                  <div className="grid grid-cols-2 gap-3">
        ...
        {step === 2 && (
          <div className="space-y-3">
            <p className="text-[10px] text-brand-navy/50">Your trade — this drives the Match% engine (35pts exp + 35pts skills).</p>
            <div className="grid grid-cols-2 gap-3">
              ...
              <div className="flex items-end pb-1"><label className="flex items-center gap-2 text-brand-navy/70 cursor-pointer"><input type="checkbox" checked={!!form.willingToTravel} onChange={e => set('willingToTravel', e.target.checked)} className="h-4 w-4 accent-brand-gold" /> Willing to relocate</label></div>
        ...
        {step === 3 && (
          <div className="space-y-3">
            <p className="text-[10px] text-brand-navy/50">Education — white-collar roles weight this (15pts).</p>
            <div className="grid grid-cols-2 gap-3">
        ...
        {step === 4 && (
          <div className="space-y-3">
            <p className="text-[10px] text-brand-navy/50">Notice period + medical fitness completes deployment readiness (15pts passport+medical).</p>
            <div className="grid grid-cols-2 gap-3">
              ...
              <div className="space-y-2">
                <label className="flex items-center gap-2 text-brand-navy/70 cursor-pointer"><input type="checkbox" checked={form.selfDeclaredFit === true} onChange={e => set('selfDeclaredFit', e.target.checked)} className="h-4 w-4 accent-brand-gold" /><span className="text-xs font-semibold">I declare myself medically fit for Gulf deployment</span></label>
                <label className="flex items-center gap-2 text-brand-navy/70 cursor-pointer"><input type="checkbox" checked={!!form.hasChronicCondition} onChange={e => set('hasChronicCondition', e.target.checked)} className="h-4 w-4 accent-brand-gold" /><span className="text-xs">Chronic condition to disclose</span></label>
              </div>
            </div>
          </div>
        )}
        {step === 5 && (
          <div className="space-y-3">
            <p className="text-[10px] text-brand-navy/50">Resume + consent — then every job card shows real Match%.</p>
            <div className="rounded-lg border border-dashed border-brand-navy/15 p-4 space-y-2 bg-brand-navy/[0.02]">
              <div className="text-[11px] font-bold text-brand-navy">📄 Resume (PDF, DOCX, JPG — max 5MB)</div>
              <label className="flex items-center gap-3 cursor-pointer">
                <span className="px-4 py-2 rounded-lg bg-brand-navy text-white text-xs font-bold">Choose file</span>
                <span className="text-xs text-brand-navy/50">{form.resumeName || 'No file selected — you can upload later in Documents'}</span>
              ...
              {form.resumeKey && <div className="text-[10px] text-emerald-700 bg-emerald-50 border border-emerald-200 rounded px-2 py-1">✓ {form.resumeName} queued — will upload on Save</div>}
            </div>
            <div className="rounded-lg border border-brand-navy/10 p-3">
              <label className="flex items-start gap-2 cursor-pointer">
                <input type="checkbox" checked={!!form.manpowerConsent} onChange={e => set('manpowerConsent', e.target.checked)} className="h-4 w-4 accent-brand-gold mt-0.5" />
                <span className="text-[10px] text-brand-navy/60 leading-relaxed"><b>Consent (DPDP 2023):</b> I allow Opus to share my profile & resume with verified Gulf employers for this application. I understand Match% is a fit estimate, not a guarantee of selection.</span>
              </label>
            </div>
            <div className="rounded-lg bg-brand-navy/[0.03] border border-brand-navy/10 p-3">
              ...
              <div className="flex flex-wrap gap-1.5">
                {comp.done.map(d => <span key={d} className="px-2 py-0.5 rounded bg-emerald-500/15 text-emerald-700 text-[9px] font-bold">✓ {d}</span>)}
                {comp.missing.map(m => <span key={m} className="px-2 py-0.5 rounded bg-brand-navy/[0.06] text-brand-navy/40 text-[9px] font-bold">○ {m}</span>)}
              </div>
            </div>
          </div>
        )}
      ...
      <div className="flex gap-3 pt-2 border-t border-brand-navy/10">
        {step > 0 ? <button onClick={() => setStep(s => s-1)} className="flex-1 border border-brand-navy/15 bg-brand-navy/[0.04] py-2.5 rounded-lg font-bold text-brand-navy cursor-pointer">← Back</button> : <div className="flex-1" />}
        {step < STEPS.length - 1 ? <button onClick={() => setStep(s => s+1)} className="flex-1 bg-brand-navy text-white py-2.5 rounded-lg font-bold cursor-pointer hover:bg-brand-navy/90">Continue →</button> : <button onClick={() => onSave(form)} disabled={saving} className="flex-1 bg-brand-gold hover:bg-brand-gold/90 text-brand-navy py-2.5 rounded-lg font-bold cursor-pointer disabled:opacity-50">{saving ? 'Saving…' : comp.pct===100 ? '✓ Save Profile' : 'Save Profile'}</button>}
      </div>
    </div>
  );
}
