...
export default function TransactionsTab() {
  ...
  const chargeLink = useMutation({
    mutationFn: async (id: string) => {
      const r = await fetch(`/api/transactions/${id}/payment-link`, { method: 'POST', headers: AUTH });
  ...
  const cancelLinkTx = useMutation({
    mutationFn: async (id: string) => {
      const r = await fetch(`/api/transactions/${id}/payment-link/cancel`, { method: 'POST', headers: AUTH });
  ...
  const renewLink = useMutation({
    mutationFn: async (id: string) => {
      const r = await fetch(`/api/transactions/${id}/payment-link/renew`, { method: 'POST', headers: AUTH });
  ...
  const { data: linkSum } = useQuery<{ outstanding: number; remindDue: number; stale: number }>({
    ...
    queryFn: async () => { const r = await fetch('/api/transactions/links-summary', { headers: AUTH }).catch(() => null); return r ? r.json() : { outstanding: 0, remindDue: 0, stale: 0 }; },
  });
  ...
  return (
    <div ref={rootRef} className="min-h-full space-y-6 text-brand-navy">
        ...
        <div className="flex items-center gap-2">
          {linkSum && linkSum.outstanding > 0 && (
            <button
              ...
              className={`flex items-center gap-1.5 rounded-full px-3 py-2 text-[10px] font-extrabold uppercase tracking-wide transition ${linkFilter === 'created' ? 'bg-brand-1 text-brand-navy' : 'bg-amber-500/15 text-amber-700 hover:bg-amber-500/30'}`}
            >
          ...
          <button onClick={openCharge} className="rounded-full bg-brand-gold px-4 py-2 text-[11px] font-extrabold uppercase tracking-wide text-brand-navy transition hover:bg-brand-gold/90">
            Charge customer
          </button>
          ...
          <label className="flex items-center gap-1.5 text-[11px] text-brand-navy/70">
      ...
      <section className="reveal rounded-2xl border border-brand-navy/10 bg-white p-6 shadow-[0_20px_40px_-20px_rgba(10,45,80,0.10)]">
        ...
        <p className="mt-0.5 text-[10px] text-brand-navy/40 mt-1">Saved as a draft — a manager confirms before it affects the client's balance.</p>
        {!isMoneyManager && type === 'invoice' && (
          <div className="mt-3 rounded-xl border border-emerald-500/30 bg-emerald-500/10 p-3 text-[11px] text-emerald-700">
        ...
        <button onClick={() => createDraft.mutate()} disabled={createDraft.isPending || !clientId || amountPaise <= 0}
          className="mt-4 rounded-full bg-brand-gold px-6 py-2.5 text-[11px] font-bold uppercase tracking-wider text-brand-navy transition hover:bg-brand-gold/90 disabled:opacity-40">
      ...
      {isMoneyManager && (
        <section className="reveal flex flex-wrap items-center justify-between gap-4 rounded-2xl border border-brand-navy/10 bg-white p-6 shadow-[0_20px_40px_-20px_rgba(10,45,80,0.10)]">
          <div>
            ...
            <p className="mt-0.5 text-[10px] text-brand-navy/40">When enabled, counselors' invoices within their division scope and under the threshold confirm + sync to ERP instantly; above it, they wait for your approval.</p>
          </div>
          <div className="flex flex-wrap items-center gap-4">
            ...
            <button onClick={() => saveProfile.mutate()} disabled={saveProfile.isPending} className="rounded-full bg-brand-gold px-5 py-2.5 text-[11px] font-bold uppercase tracking-wider text-brand-navy hover:bg-brand-gold/90 disabled:opacity-40">
            ...
            {rows.map((t) => (
              <tr key={t.id} className="border-b border-brand-navy/[0.08] last:border-0 hover:bg-brand-navy/[0.04]">
                <td className="px-5 py-3.5">
                  ...
                  <div className="font-mono text-[9px] text-brand-navy/50">{t.clientId}</div>
                </td>
                <td className="px-5 py-3.5"><span className="rounded-full border border-brand-navy/10 bg-brand-navy/[0.06] px-2 py-0.5 text-[9px] font-bold uppercase text-brand-navy/70">{TYPE_LABEL[t.type] || t.type}</span></td>
                ...
                <td className="px-5 py-3.5 text-[10px] text-brand-navy/50">
                  ...
                  {(t.type === 'invoice' || t.type === 'charge') && t.status !== 'paid' && t.status !== 'void' && (
                    t.linkStatus === 'none' ? (
                      <button onClick={() => chargeLink.mutate(t.id)} disabled={chargeLink.isPending}
                        className="rounded-full bg-brand-gold px-2.5 py-1 text-[9px] font-bold uppercase text-brand-navy transition hover:bg-brand-gold/90 disabled:opacity-40">
                      ...
                      <span className="flex flex-col gap-1">
                        <span className="flex flex-wrap items-center gap-1.5">
                          {t.linkStatus === 'created' && (
                            <button onClick={() => cancelLinkTx.mutate(t.id)} disabled={cancelLinkTx.isPending} className="rounded-full border border-rose-500/40 px-2 py-0.5 text-[9px] font-bold uppercase text-rose-700 hover:bg-rose-600 hover:text-white">cancel</button>
                          )}
                          {(t.linkStatus === 'cancelled' || t.linkStatus === 'expired') && (
                            <button onClick={() => renewLink.mutate(t.id)} disabled={renewLink.isPending} className="rounded-full border border-amber-500/50 px-2 py-0.5 text-[9px] font-bold uppercase text-amber-700 hover:bg-amber-500 hover:text-white">renew</button>
                          )}
                          <span className={`rounded-full px-2 py-0.5 text-[9px] font-bold uppercase ${t.linkStatus === 'paid' ? 'bg-emerald-500/15 text-emerald-700' : t.linkStatus === 'cancelled' || t.linkStatus === 'expired' ? 'bg-rose-500/15 text-rose-700' : 'bg-amber-500/15 text-amber-700'}`}>
                            {t.linkStatus}{t.razorpayPaymentId ? ` · ${t.razorpayPaymentId.slice(0, 10)}` : ''}
                          </span>
                          {t.linkStatus === 'created' && (
                            <span className={`rounded-full px-2 py-0.5 text-[9px] font-bold ${ageClass(ageDays(t))}`}>{ageDays(t)}d</span>
                          )}
                        </span>
                        {t.razorpayShortUrl && t.linkStatus !== 'paid' && (
                          <span className="flex gap-1.5">
                            <button onClick={() => copyUrl(t.razorpayShortUrl!)} className="text-[9px] font-bold text-brand-gold hover:underline">copy</button>
                            <a href={t.razorpayShortUrl} target="_blank" rel="noopener noreferrer" className="text-[9px] font-bold text-emerald-700 hover:underline">view</a>
                          </span>
                        )}
                      </span>
                    )
                ...
                <td className="px-5 py-3.5 text-[10px] text-brand-navy/50">{t.enteredBy ? t.enteredBy.slice(0, 8) : '—'}</td>
                <td className="px-5 py-3.5">
                  ...
                  {!isMoneyManager && t.status === 'draft' && <span className="text-[9px] text-brand-navy/50">awaiting confirm</span>}
                </td>
              </tr>
            ))}
          </tbody>
      ...
      {chargeOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4 backdrop-blur-sm" onClick={() => setChargeOpen(false)}>
          <div className="w-full max-w-md rounded-2xl border border-brand-navy/10 bg-white p-6 shadow-2xl" onClick={(e) => e.stopPropagation()}>
            ...
            <p className="mt-1 text-[10px] leading-relaxed text-brand-navy/40">
            ...
            {!chargeResult ? (
              <div className="mt-4 space-y-3">
                ...
                <div>
                  <label className="mb-1 block text-[9px] font-bold uppercase tracking-wider text-brand-gold">What's it for</label>
                ...
                {chargeClientId && <p className="text-[10px] text-brand-navy/50">Attached to engagement: <span className="font-semibold">{chargeEngLabel || '(auto — first active)'}</span></p>}
                <button
                  onClick={() => chargeAny.mutate()}
                  disabled={!chargeClientId || !chargeAmount || Number(chargeAmount) < 1 || chargeAny.isPending}
                  className="w-full rounded-full bg-brand-gold px-4 py-2.5 text-[11px] font-extrabold uppercase tracking-wide text-brand-navy transition hover:bg-brand-gold/90 disabled:opacity-40"
                >
              ...
              <div className="mt-4 rounded-xl border border-emerald-500/30 bg-emerald-500/10 p-4">
                <p className="text-[11px] font-bold text-emerald-700">Link created — send it to the customer.</p>
                <div className="mt-2 flex items-center gap-2 rounded-lg bg-brand-navy/[0.04] p-2 ring-1 ring-emerald-500/30">
                  <span className="flex-1 truncate font-mono text-[10px] text-brand-navy/70">{chargeResult.shortUrl}</span>
                  <button onClick={() => copyUrl(chargeResult.shortUrl!)} className="text-[10px] font-bold text-brand-gold hover:underline">Copy</button>
                </div>
                <div className="mt-3 flex gap-2">
                  <a href={`https://wa.me/?text=${encodeURIComponent('Please pay via this link: ' + chargeResult.shortUrl)}`} target="_blank" rel="noopener noreferrer" className="flex-1 rounded-full bg-emerald-600 px-3 py-2 text-center text-[10px] font-bold uppercase text-white transition hover:bg-emerald-700">WhatsApp</a>
                  <a href={chargeResult.shortUrl} target="_blank" rel="noopener noreferrer" className="flex-1 rounded-full border border-brand-navy/15 bg-brand-navy/[0.04] px-3 py-2 text-center text-[10px] font-bold uppercase text-brand-navy transition hover:border-brand-gold/50">Open link</a>
                </div>
                <button onClick={() => { setChargeResult(null); setChargeAmount(''); setChargeDesc(''); setChargeClientId(''); }} className="mt-3 w-full text-[10px] font-bold text-brand-navy/50 hover:text-brand-gold">New charge</button>
              </div>
            )}
          </div>
        </div>
