...
function RiskBadge({ score, flags, verified }: { score: number; flags: string[]; verified: boolean }) {
  if (verified) return <span className="px-1.5 py-0.5 rounded text-xs font-bold uppercase bg-emerald-500/15 text-emerald-700">✓ Verified</span>;
  if (score >= 50) return <span className="px-1.5 py-0.5 rounded text-xs font-bold uppercase bg-rose-500/15 text-rose-600" title={flags.join(', ')}>🚨 High risk {score}</span>;
  if (score >= 20) return <span className="px-1.5 py-0.5 rounded text-xs font-bold uppercase bg-amber-500/15 text-amber-700" title={flags.join(', ')}>⚠ Review {score}</span>;
  return <span className="px-1.5 py-0.5 rounded text-xs font-bold uppercase bg-emerald-500/15 text-emerald-700">🟢 Genuine</span>;
}
...
function StatusBadge({ status }: { status: string }) {
  ...
  return <span className={`px-1.5 py-0.5 rounded text-xs font-bold uppercase ${map[status] || 'bg-brand-navy/[0.06] text-brand-navy/50'}`}>{status}</span>;
}
  ...
  return (
    <div className="flex justify-between items-center bg-brand-navy/[0.06] border border-brand-navy/10 rounded px-3 py-2 text-xs">
      <div>
        ...
        {flags.length > 0 && <div className="flex flex-wrap gap-1 mt-1">{flags.map((f: string) => <span key={f} className="px-1 py-0.5 rounded bg-brand-navy/[0.06] text-sm font-bold uppercase text-brand-navy/50">{f.replace(/_/g, ' ')}</span>)}</div>}
      </div>
      <div className="flex items-center gap-2 shrink-0">
        ...
        {onVerify && !b.verified && b.status === 'scheduled' && (
          <button onClick={() => onVerify(b.id)} className="border border-emerald-600/40 px-2 py-0.5 rounded text-xs font-bold uppercase text-emerald-700 hover:bg-emerald-600 hover:text-white transition cursor-pointer">Verify</button>
        )}
      </div>
    </div>
  );
    ...
    <div ref={rootRef} className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <div className="flex items-center gap-2">
            ...
            <span className="text-[13px] font-bold uppercase tracking-[0.18em] text-brand-gold">Consultations</span>
          </div>
      ...
      {showCfg && (
        <div className="reveal rounded-2xl border border-brand-navy/10 bg-white p-5 space-y-3">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-2">
            <div>
              <label className="text-[13px] text-brand-navy/40 font-bold uppercase block mb-1">API key {cfg?.apiKey ? <span className="text-emerald-600">(saved {cfg.apiKey})</span> : null}</label>
            ...
            <div>
              <label className="text-[13px] text-brand-navy/40 font-bold uppercase block mb-1">Webhook secret</label>
            ...
            <div>
              <label className="text-[13px] text-brand-navy/40 font-bold uppercase block mb-1">Event types JSON (division → eventTypeId)</label>
            ...
            <div>
              <label className="text-[13px] text-brand-navy/40 font-bold uppercase block mb-1">Booking links JSON (division → cal.com URL)</label>
            ...
            <div>
              <label className="text-[13px] text-brand-navy/40 font-bold uppercase block mb-1">Pending-booking email alert →</label>
            ...
            <div>
              <label className="text-[13px] text-brand-navy/40 font-bold uppercase block mb-1">Pending-booking WhatsApp alert →</label>
          ...
          <p className="text-[13px] text-brand-navy/40">Webhook URL: <code className="font-mono">https://&lt;api&gt;/api/webhooks/cal</code> — set it in cal.com → Settings → Developer → Webhooks with triggers: booking.created / cancelled / rescheduled / meeting.ended.</p>
        </div>
      )}

      {/* Risk filter */}
      <div className="flex flex-wrap items-center gap-2">
        <span className="text-[13px] font-bold uppercase text-brand-navy/40">Risk filter:</span>
        {[['all', 'All'], ['high', '🚨 High risk'], ['review', '⚠️ Review'], ['genuine', '🟢 Genuine']].map(([k, label]) => (
          <button key={k} onClick={() => setRiskFilter(k)} className={`px-3 py-1.5 rounded-full text-[13px] font-bold transition-all cursor-pointer ${riskFilter === k ? 'bg-brand-gold text-brand-navy' : 'border border-brand-navy/15 text-brand-navy/50 hover:text-brand-navy'}`}>{label}</button>
        ))}
        <span className="text-[13px] text-brand-navy/40 ml-auto">Anti-spam: email verification + 1/day limit + phone + qualifying question (cal.com) · suspicion scoring + flood blocking (OS)</span>
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        {[
          ...
          <div key={k.label} className="rounded-2xl border border-brand-navy/10 bg-white p-4">
            <div className="text-xs font-bold uppercase tracking-[0.18em] text-brand-navy/40">{k.label}</div>
      ...
      <div className="reveal rounded-2xl border border-brand-navy/10 bg-white p-5 space-y-3">
        ...
        <div className="space-y-1.5">
          ...
          {(data?.today || []).length === 0 && <p className="text-[13px] text-brand-navy/40 text-center py-2">No consultations today.</p>}
        </div>
      </div>
      ...
      <div className="reveal rounded-2xl border border-brand-navy/10 bg-white p-5 space-y-3">
        ...
        <div className="space-y-1.5 max-h-72 overflow-y-auto pr-1">
          ...
          {(data?.upcoming || []).length === 0 && <p className="text-[13px] text-brand-navy/40 text-center py-2">No upcoming consultations.</p>}
        </div>
      </div>
      ...
      <div className="reveal rounded-2xl border border-brand-navy/10 bg-white p-5 space-y-3">
        ...
        <div className="space-y-1.5 max-h-72 overflow-y-auto pr-1">
          ...
          {(data?.past || []).length === 0 && <p className="text-[13px] text-brand-navy/40 text-center py-2">No past bookings yet.</p>}
        </div>
      </div>
    </div>
  );
